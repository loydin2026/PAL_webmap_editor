...
function bindDraggablePanel() {
    ...
    try {
      ...
      for (const name of gopList) {
        if (gopCache[name] && gopCache[name].tiles) {
          ...
          subfiles.push(new Uint8Array(buf));
        }
      }
      ...
      a.click(); URL.revokeObjectURL(url);
      updateStatus('GOP.MKF 已导出 (' + gopList.length + ' 个子文件, 含当前编辑状态)');
    } catch (e) {
  ...
  // 将目录中所有 MAP 文件打包为 MKF（导出当前编辑状态，跳过全 0 空占位符）
  async function exportMapToMkf() {
    ...
    try {
      ...
      a.click(); URL.revokeObjectURL(url);
      updateStatus('MAP.MKF 已导出 (' + mapList.length + ' 个子文件, 跳过 ' + skipped + ' 个空占位符)');
    } catch (e) {
